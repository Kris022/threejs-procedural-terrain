...
let n2 = createNoiseMap(100, 4.95, 0);
const terrain2 = mapToMesh(n2);
terrain2.position.setX(100);
scene.add(terrain2);
/*
...
function isOutOfBounds(orgin) {
  if (camera.position.z >= 50) {
    return true;
  } else if (camera.position.z <= -50) {
    const n = createNoiseMap(100, 0, origin - 4.95);
    const chunk = mapToMesh(n);
    chunk.position.setZ(origin - 100);
    scene.add(chunk);
    return true;
  } else {
    return false;
...
function onKeyDown(e) {
  ...
  console.log(Math.round(camera.position.z / 100));
}
